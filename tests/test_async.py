...
from oracledb import AsyncConnection, SessionPool as AsyncConnectionPool
...
from oracle.aio import (
    AsyncOracleSaver,
    AsyncShallowOracleSaver,
)
from tests.conftest import DEFAULT_ORACLE_CONFIG
...
async def _pool_saver():
    ...
    try:
        # yield checkpointer
        checkpointer = AsyncOracleSaver(pool)
...
async def _pipe_saver():
    ...
    try:
        checkpointer = AsyncOracleSaver(conn)
        await checkpointer.setup()
        async with conn.pipeline() as pipe:
            checkpointer = AsyncOracleSaver(conn, pipe=pipe)
            yield checkpointer
    finally:
        await conn.close()
...
async def _shallow_saver():
    ...
    try:
        checkpointer = AsyncShallowOracleSaver(conn)
        await checkpointer.setup()
        yield checkpointer
    finally:
        await conn.close()
